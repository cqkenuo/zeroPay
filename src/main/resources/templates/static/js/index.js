    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.isSuccess == true) {
                window.location = '/indexdata';
            } else {
                layer.msg('登陆失败，账户或者密码错误', {icon: 5});
                // $('#code').trigger('click');
                // msg_alert(data.message);
            }
        }
...
function login() {
    ...
    var thisform = document.forms['formlogin'];
    var loginName = thisform.loginName.value;
...
function adduser() {
    ...
    var country = thisform.countryid.value;
